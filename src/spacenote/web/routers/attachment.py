...
class AttachmentPageRouter(View):
    ...
    async def assign_attachment_to_note(self, space_id: str, attachment_id: int, note_id: int) -> RedirectResponse:
        ...
        await self.app.assign_attachment_to_note(self.current_user, space_id, attachment_id, note_id)
    ...
    async def get_note_attachments(self, space_id: str, note_id: int) -> HTMLResponse:
        ...
        attachments = await self.app.get_note_attachments(self.current_user, space_id, note_id)
    ...
    async def show_assign_form(self, space_id: str, attachment_id: int) -> HTMLResponse:
        ...
        space = self.app.get_space(self.current_user, space_id)
        attachment = await self.app.get_attachment(self.current_user, space_id, attachment_id)
        notes_result = await self.app.list_notes(self.current_user, space_id, page=1, page_size=1000)
    ...
    async def delete_attachment(self, space_id: str, attachment_id: int) -> RedirectResponse:
        ...
        await self.app.delete_attachment(self.current_user, space_id, attachment_id)
        return redirect(f"/attachments/spaces/{space_id}")
